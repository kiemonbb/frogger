...
{
    usleep(FRAME_TIME * 1000-(startFrame-endFrame)); // program sleeps FRAME_TIME[ms] between frames
    timer->frame_no++;
...
{
    mvwprintw(status->win, status->height / 2, status->width / 2 - 10, "Time elapsed: %.2fs", timer->time_elapsed / 1000);
    wrefresh(status->win);
}
